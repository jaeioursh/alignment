...
def load_data(n_agents=5,agent_idx=0,n_actors=4,iteration=0,generation=0,q=3,hidden=20):
    ...
    fname="tests/"+str(n_agents)+"-"+str(n_actors)+"-"+str(iteration)+"-"+str(q)+"/"+str(generation)
    ...
    net=Net(hidden)
    net.model.load_state_dict(torch.load(fname+".mdl")[agent_idx])
    return env,pos,teams,net

#env.reset()
def eval(x,y,t,team_idx,agent_idx,env,pos,teams,time=-1):
    test=pos[time][team_idx].copy()
    test[teams[team_idx]==agent_idx,:]=[x,y]
...
if __name__=="__main__":
    agent_idx=0
    team_idx=0
    env,pos,teams,net=load_data(n_agents=5,agent_idx=agent_idx,n_actors=4,iteration=0,generation=0)
    x=0 # -5 to 35 ish
    y=0 # -5 to 35 ish
    t=0 #-pi to pi
