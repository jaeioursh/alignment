...
def eval(team_idx,agent_idx,env,position,teams,generation,time=-1):
    pos,rot=position[generation][team_idx][time]
    pos,rot=pos.copy(),rot.copy()
    #pos[teams[team_idx]==agent_idx,:]=[,]
    env.data["Agent Orientations"]=rot
    #env.data["Agent Orientations"][teams[team_idx]==agent_idx,:]=[np.sin(),np.cos()]
...
if __name__=="__main__":
    ...
    state,G = eval(team_idx,agent_idx,env,pos,teams,generation,time=3)
